...
def compact_ledger(
    ...
    for key, value in data.items():
        ...
        eligible = []
        for item in value:
            identity = re.fullmatch(r"(?:E(\d+)-)?S(\d+)", str(item.get("scene_id", ""))) if isinstance(item, dict) else None
            if identity and target_start is not None:
                episode = int(identity.group(1) or 0)
                scene = int(identity.group(2))
                is_future = scene > (target_end or target_start) if target_episode is None else (episode, scene) > (target_episode, target_end or target_start)
                if is_future:
                    continue
            ...
            if target_start is not None and isinstance(item, dict):
                ...
                scene_match = bool(
                    match
                    and (target_episode is None or not identity or int(identity.group(1) or 0) == target_episode)
...
def main() -> int:
    ...
    def add(path: Path, priority: int, mode: str = "full") -> None:
        ...
        if not path.is_file() or path.suffix.lower() not in ALLOWED_SUFFIXES:
            return
        if target_start is not None and "screenplay/scenes/" in path.relative_to(root).as_posix():
            try:
                _, episode, number = scene_identity(path)
                upper = target_end or target_start
                if (episode or 0, number) > (args.episode or 0, upper + int(args.include_next_scene)):
                    return
            except ValueError:
                return
        text = safe_read(path)
        ...
        if mode == "scene":
            ...
            text = compact_ledger(text, terms, target_start, target_end, args.episode)
    ...
    harness_budget = min(4500, max(700, int(max_chars * 0.28)))
    try:
        harness_context = render_harness_context(root, terms, max_chars=harness_budget)
    except (OSError, ValueError) as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        return 2
    parts = [header]
